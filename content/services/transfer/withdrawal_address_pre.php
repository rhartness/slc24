...
$_PRE["links"] = false;
$_PRE["account"] = true;
$_PRE["page_title"] = "Transfer withdrawal address - Solidcoin24";
$_PRE["title"] = "Solidcoin24 Transfer";
$_PRE["sidemenu_file"] = "transfer_menu.php";

$address = mysql_real_escape_string($_GET["a"], $db);

$slt_transfer_withdrawal_address_a = "SELECT *, UNIX_TIMESTAMP(creation_time) AS creation_time_u FROM transfer_withdrawal_address WHERE address = '$address'";
...
if ($row_transfer_withdrawal_address_a = mysql_fetch_assoc($rlt_transfer_withdrawal_address_a)) {
	$_PRE["REQUEST_URI"] = "/?c=services/transfer/withdrawal_address&a=".urlencode($address);
}

if ($row_transfer_withdrawal_address_a["user"] != $_SESSION["user_id"]) {
	$_PRE["REQUEST_URI"] = "/?c=services/transfer/withdrawal_address";
}
