...
$slt_transfer_deposit_a = "SELECT t.id AS id, a.address AS address, t.type AS type, t.amount AS amount, UNIX_TIMESTAMP(t.filing_time) AS filing_time_u, t.txid AS txid, a.id AS aid FROM transfer_deposit t, transfer_deposit_address a WHERE a.user = '$_SESSION[user_id]' AND t.deposit_address = a.id";
if (isset($type))
	$slt_transfer_deposit_a .= " AND t.type = '$type'";
$slt_transfer_deposit_b = $slt_transfer_deposit_a;
$slt_transfer_deposit_a .= " ORDER BY t.filing_time DESC, t.id DESC LIMIT $from,$entries";
